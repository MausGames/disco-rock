...
void FragmentMain()
{
    ...
    else
    {
        ...
        gl_FragColor      = vec4(u_v4Color.rgb * (0.9 * v1Intensity * min(u_v4Color.a * v1Intensity, 1.0)), 1.0);
    }
    ...
    float v1Intensity = min(406.25 * v1Rsqrt * min(abs(dot(v_v3Relative * v1Rsqrt, c_v3CamDir)), 1.0) - 0.4063, 1.25);
    ...
    vec4 v4Color   = vec4(vec3(v2TextureColor.r), 1.0) * u_v4Color * (v1Intensity * (0.9 * v1BumpFactor + 0.25 * pow40(v1BumpFactor)));
    vec3 v3Diffuse = v4Color.rgb * min(v4Color.a, 1.0);

    #if (_CORE_QUALITY_) > 1
    
        // add disco light effect
        v3Diffuse += vec3(v1TextureDisco * (0.5075 - 0.435 * sin(v_av2TexCoord[0].y * 0.25 * PI) * sin(v_av2TexCoord[1].x * 0.5 * PI)));
